...
from PersonalizedAntibiogramMatrix import PersonalizedAntibiogramMatrix

class PersonalizedAntibiogramPredictionPipeline(SupervisedLearningPipeline):
    ...
    def _build_model_dump_path(self, algorithm):
        template = '%s' + '-PA-%s-model.pkl' % algorithm
    ...
    def _build_raw_matrix_path(self):
        template = '%s-PA-matrix-%d-episodes-raw.tab'
        pipeline_file_name = inspect.getfile(inspect.currentframe())
    ...
    def _build_raw_feature_matrix(self):
        ...
        matrix_class = PersonalizedAntibiogramMatrix
        params = None
        if params is None:
    ...
    def _build_processed_matrix_path(self):
        template = '%s-PA-matrix-%d-episodes-processed.tab'
    ...
    def _build_processed_feature_matrix(self):
        ...
        features_to_remove = [
            'pat_id', 'shifted_order_time', 'organism_name', 'pat_enc_csn_id',
...
if __name__ == '__main__':
    ...
    PersonalizedAntibiogramPredictionPipeline("PersonalizedAntibiogram", panel, 1000000, use_cache=True, random_state=123456789)
