...
class AntiBiogramMatrix(FeatureMatrix):
    def __init__(self, bug_panel, num_episodes, random_state=None):
        FeatureMatrix.__init__(self, bug_panel, num_episodes)

        # Parse arguments.
        self._bug_panel = bug_panel
        self._lab_panel = ['LABBLC', 'LABBLC2', 'LABURNC'] # we will use prior blood and urine cultures as features
    ...
    def _query_patient_episodes(self):
        ...
        susceptibility_flags = ['Piperacillin/Tazobactam', 'Meropenem', 'Imipenem', 'Ceftriaxone', 'Cefazolin', 'Aztreonam.']

        for med in susceptibility_flags:
            med_col = med.replace('/', '_').replace('.', '')
            if med == 'Cefazolin':
                query.addSelect("MAX(CASE WHEN (antibiotic_name = '%s' or antibiotic_name = '%s.') AND (suseptibility = 'Susceptible' OR suseptibility = 'Positive') THEN 1 ELSE 0 END) AS %s_Susc" % (med, med, med_col))
                query.addSelect("MAX(CASE WHEN (antibiotic_name = '%s' or antibiotic_name = '%s.') THEN 1 ELSE 0 END) as %s_tested" % (med, med, med_col))
            else:
        ...
        query.addWhereIn("stride_culture_micro.organism_name", [self._bug_panel])
        query.addWhereIn("stride_culture_micro.antibiotic_name", ['Piperacillin/Tazobactam', 'Meropenem', 'Imipenem', 'Ceftriaxone', 'Cefazolin', 'Cefazolin.', 'Aztreonam.'])
