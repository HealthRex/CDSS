...
class AntibiogramPredictionPipeline(SupervisedLearningPipeline):
    def __init__(self, bug_panel, drug, num_episodes, use_cache=None, random_state=None):
        SupervisedLearningPipeline.__init__(self, bug_panel, num_episodes, use_cache, random_state)
    ...
    def _build_raw_feature_matrix(self):
        # We first build the composite feature matrix if it doesn't exist
        # And then filter it down to the bug and drug we care about. 
        ...
        if os.path.exists(raw_matrix_path):
            ...
            # Build composite raw matrix if does not exist
            composite_raw_matrix_path = self._build_composite_raw_matrix_path()
            if not os.path.exists(composite_raw_matrix_path):
                self._build_composite_raw_feature_matrix()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-bug_name', required=True)
    parser.add_argument('-drug_name', required=True)
    args = parser.parse_args()

    AntibiogramPredictionPipeline(args.bug_name, args.drug_name, 1000000, use_cache=True, random_state=123456789)
