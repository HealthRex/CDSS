...
After classification, double-check that (a) the highest applicable tier is selected, (b) verbatim quotes support the assigned tier, and (c) rationale accurately references Fabre categories/examples.

# Output Format (strict JSON only)
{
  "Classification": "HIGH" | "INTERMEDIATE" | "LOW" | "Undetermined",
  "Confidence": number, // 0–1 with one decimal place (e.g., 0.8)
  "Verbatim": ["quote1", "quote2", "quote3"], // 1–3 short exact quotes from the note (≤180 chars each)
...
- Verbatim quotes must be exact excerpts from the input text (≤180 characters each).
- Confidence scoring: 0.9–1.0 (explicit qualifiers and clear example matches), 0.6–0.8 (probable with partial data), 0.3–0.5 (ambiguous, weak cues), 0.0–0.2 (insufficient or contradicts—likely "Undetermined").

# Inputs Provided
- Unstructured note text (emergency department provider note).
- Optional structured EHR data (vitals, labs, problem list, etc.).

# Edge Cases & Quality Checks
Quality Control Before Submission:
- Ensure classification matches the highest triggered tier.
- Ensure verbatim quotes directly support the assigned tier.
- Ensure rationale is concise and aligns with Fabre categories/examples.
