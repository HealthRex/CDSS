#!/usr/bin/env python3
"""
Runner for the Fabre minimal batch classifier (JSON-aware).
...
- Output dir: .../src/data/EHR_IncludeInprompt_<flag>_fabre_minimal_v2/
...
from parallel_classify_minimal_fabre_v2 import MinimalAsyncClassifierFabreV2


async def main():
    parser = argparse.ArgumentParser(description="Run minimal Fabre LLM classification (JSON-aware)")
    parser.add_argument(
        "--source-csv",
        default=None,
    ...
    if args.data_dir is None:
        args.data_dir = os.path.join(
            base_data_dir, f"EHR_IncludeInprompt_{args.EHR_IncludeInprompt}_fabre_minimal_v2"
        )

    print("🚀 Fabre minimal classification runner (v2, JSON-aware)")
