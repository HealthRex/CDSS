...
from scripts.LabTestAnalysis.machine_learning.extraction.ComponentNormalityMatrix import ComponentNormalityMatrix

class ComponentNormalityPredictionPipeline(SupervisedLearningPipeline):
    def __init__(self, component, num_episodes, use_cache=None, random_state=None):
        SupervisedLearningPipeline.__init__(self, component, num_episodes, use_cache, random_state)
    ...
    def _build_model_dump_path(self, algorithm):
        template = '%s' + '-component-normality-%s-model.pkl' % algorithm
    ...
    def _build_raw_matrix_path(self):
        template = '%s-component-normality-matrix-%d-episodes-raw.tab'
        pipeline_file_name = inspect.getfile(inspect.currentframe())
    ...
    def _build_processed_matrix_path(self):
        template = '%s-component-normality-matrix-%d-episodes-processed.tab'
    ...
    def _train_and_analyze_predictors(self):
        ...
        for algorithm in algorithms_to_test:
            ...
            if status == SupervisedClassifier.INSUFFICIENT_SAMPLES:
                ...
                header = ['ComponentNormalityPredictionPipeline("%s", 10000)' % self._var]
                # Write error report.
                fm_io.write_data_frame_to_file(algorithm_report, \
                    '/'.join([report_dir, '%s-component-normality-prediction-report.tab' % (self._var)]), \
                    header)
            # If successfully trained, append to a meta report.
            elif status == SupervisedClassifier.TRAINED:
                pipeline_prefix = '%s-component-normality-prediction-%s' % (self._var, algorithm)
        ...
        if meta_report is not None:
            ...
            fm_io.write_data_frame_to_file(meta_report, \
                '/'.join([data_dir, '%s-component-normality-prediction-report.tab' % self._var]), header)
...
if __name__ == '__main__':
    ...
    for component in components_to_test:
        ComponentNormalityPredictionPipeline(component, 10000, use_cache=True, random_state=123456789)
