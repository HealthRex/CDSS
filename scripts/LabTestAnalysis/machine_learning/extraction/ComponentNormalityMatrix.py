...
class ComponentNormalityMatrix(FeatureMatrix):
    def __init__(self, component, num_episodes, random_state=None):
        FeatureMatrix.__init__(self, component, num_episodes)
    ...
    def _add_features(self):
        ...
        #sx
        log.info('Adding component item features...')
        #
        # self._factory.addClinicalItemFeatures([self._lab_panel], features="pre")
        self._factory.addClinicalItemFeatures([self._component], features="pre")
        log.info('addClinicalItemFeatures Finished!')
        ...
        for pre_time_delta in LAB_PRE_TIME_DELTAS:
            ...
            self._factory.addLabResultFeatures([self._component], False, pre_time_delta, LAB_POST_TIME_DELTA) #sx
    ...
    def _build_data_overview(self):
        ...
        line = 'the %s lab panel across %s inpatients from Stanford hospital.' % (self._component, self._num_patients) #sx
        overview.append(line)
    ...
    def _build_field_summary(self):
        ...
        line = '%s.[clinical_item] - orders of the component of interest.' % self._component #sx
        summary.append(line)
        ...
        line = 'Also included %s panel components: %s' % (self._component, self._component)
        summary.append(line)
...
if __name__ == "__main__":
    ...
    ltm = ComponentNormalityMatrix(options.lab, options.numRows, options.randomState)
