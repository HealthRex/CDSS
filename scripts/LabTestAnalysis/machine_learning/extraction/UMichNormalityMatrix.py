...
class UMichNormalityMatrix(FeatureMatrix):
    ...
    def _get_random_patient_list(self):
        ...
        if len(results) == 0:
            error_msg = '0 orders for order "%s."' % self._lab_panel #sx
            log.critical(error_msg)
            sys.exit('[ERROR] %s' % error_msg)
        else:
        ...
        # print 'random_patient_list:',random_patient_list
        # quit()
        #
    ...
    def _query_patient_episodes(self):
        ...
        #           metabolic components. Include it.
        query = SQLQuery()
        query.addSelect('CAST(pat_id AS BIGINT) AS pat_id') #TODO
        # query.addSelect('pat_id')
        query.addSelect('order_proc_id')
        ...
        query.addSelect("SUM(CASE WHEN result_in_range_yn = 'Y' THEN 1 ELSE 0 END) AS num_normal_components") #OR result_in_range_yn IS NULL
        ...
        query.addSelect("CAST(SUM(CASE WHEN result_in_range_yn = 'N' THEN 1 ELSE 0 END) = 0 AS INT) AS all_components_normal")
        query.addFrom('labs')
    ...
    def _add_features(self):
        ...
        self._factory.addClinicalItemFeatures([self._lab_panel], features="pre", is_Michigan_data=True,
                                              clinicalItemType='proc_code', clinicalItemTime='order_time', tableName='labs') #sx
