...
def main(argv):
    ...
    extractor = DataExtractor("tempOutputFileName");
    ...
    "order_proc_id": "1",
    "proc_code": "777",
    "order_time": "2013-07-04 00:00:00",
    "abnormal": 1,
    "result_normal_count": 0,
    ...
    #outFile = sys.stdout;
    outFile = stdOpen("labFeatureMatrix.newDataExtractor.%s.tab.gz" % fileLabel,"w");
    ...
    print extractor.features["timeCycle"].timeAttributes

    # Added for new DataExtractor to work
    extractor.writeMatrixFile(formatter)
...
def queryPatientEpisodes(outputFile, labProcCode, startDate, endDate, extractor):
    ...
    try:
        ...
        cohortQueryString =  DBUtil.parameterizeQueryString(cohortQuery)
...
def populatePatientData(patientEpisodeByIndexTime, extractor):
    ...
    if patientId is not None:
        ...
        eventTimes = extractor.parseClinicalItemData_singlePatient( modelListFromTable(extractor.queryClinicalItemsByName(("Birth",), [patientId], prefix="Birth", daysBins=[])) );
        tempColNames.extend( extractor.addClinicalItemFeatures_singlePatient(eventTimes, patientEpisodeByIndexTime, "Birth", daysBins=[]) );
        eventTimes = extractor.parseClinicalItemData_singlePatient( modelListFromTable(extractor.queryClinicalItemsByName(("Male",), [patientId], prefix="Male", daysBins=[])) );
        tempColNames.extend( extractor.addClinicalItemFeatures_singlePatient(eventTimes, patientEpisodeByIndexTime, "Male", daysBins=[]) );
        eventTimes = extractor.parseClinicalItemData_singlePatient( modelListFromTable(extractor.queryClinicalItemsByName(("Female",), [patientId], prefix="Female",daysBins=[])) );
        ...
        eventTimes = extractor.parseClinicalItemData_singlePatient( modelListFromTable(extractor.queryClinicalItemsByCategory((ADMIT_DX_CATEGORY_ID,), [patientId],prefix="AdmitDxDate",daysBins=[])) );
