...
def line_str2list(line_str, params_str2list=None, is_column=False):
...
def perturb_a_file(raw_file_path, target_file_path, col_patid, perturb_dict, params_str2list):
    ...
    for line_raw in lines_raw[1:]:
        ...
        line_as_list = line_str2list(line_raw, params_str2list)
        try:
...
def create_large_files(raw_data_files,raw_data_folderpath,
    ...
    all_pat_ids = set([line_str2list(line,params_str2list=params_str2list)[col_patid] for line in lines_lab[1:]]) # skip the first row as columns
...
def pd2db(data_df, df_name, db_path, db_name, data_source):
...
def raw2db(data_file, data_folderpath, db_path, db_name,
    ...
    with open(data_folderpath + '/' + data_file) as f:
        ...
        while True:
            ...
            generated_tables += pd2db(data_df, df_name=df_name, db_path=db_path, db_name=db_name, data_source=data_source)
            ##
