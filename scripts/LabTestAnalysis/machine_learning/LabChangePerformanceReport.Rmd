...
component_map$description <- c("Creatinine kinase", "C-reactive protein", "Ferritin", "Heparin Activity",
                               ...
                               "Phosphate in serum/plasma", "Platelet count", "Partial Thromboplastin Time",
...
load_and_process <- function(lab, params = 1:5*10, n_models = 6, change_def = "percent"){
  ...
  return(preprocess_data(raw, lab, params, n_models) %>% filter(!grepl("L1_REGRESS_AND_ROUND",model)))
}
```
...
summarize_group_report <- function(full_data){
  ...
  summarize(max_roc = max(roc_auc),
            model = nth(model, which.max(percent_predictably_positive)),
...
predictable_vol_bar_plot <- function(report_summary, fill_label = "Change Threshold", mypalette = "Reds"){
  plotdata <- mutate(report_summary, 
                     volume = max_percent_predictability*num_repeated)
  
  myplot <- ggplot(plotdata, aes(description, ifelse(volume>1, volume, 1), fill = as.factor(change_param))) +
    geom_bar(position = "dodge", stat = "identity") +
    labs(fill = fill_label, y = "Predictable volume (log scale)", x = NULL) +
    geom_text(aes(label=comma(round(signif(volume,2)))),
              hjust = -0.05, position=position_dodge(width=0.9), size = 3) +
    coord_flip() +
    scale_fill_brewer(palette = mypalette, direction= -1) +
    theme_bw() +
    scale_y_log10(labels = comma, limits = c(1, 2000000)) +
...
predictable_charge_bar_plot <- function(report_summary, fill_label = "Change Threshold", mypalette = "Reds"){
  plotdata <- mutate(report_summary, 
                     volume = max_percent_predictability*num_repeated*median_price)
  myplot <- ggplot(plotdata, aes(description, 
                                       ifelse(volume >1, volume, 1),
                                       fill = as.factor(change_param))) +
    geom_col(position = "dodge") +
    labs(fill = fill_label, y = "Predictable charge (log scale)", x = NULL) +
    coord_flip() +
    scale_fill_brewer(palette = mypalette, direction= -1) +
    geom_text(aes(label=paste("$", comma(round(signif(volume,2))), sep = "")),
