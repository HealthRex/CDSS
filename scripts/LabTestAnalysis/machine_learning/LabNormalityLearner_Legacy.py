...
import LocalEnv
import os
from medinfo.ml import SupervisedLearner_Legacy as SL
...
def main_pipelining(labs,
    ...
    data_set_foldername = 'data-%s-%s-%s-episodes ' % \
    (data_source, lab_type, num_episodes) # TODO: template shared by stats module
...
def apply_src_to_dst(lab, lab_type,
    ...
    patIds_evalu_dst = ml_utils.get_patIds \
        (df_processed_evalu_dst)  # set(df_ucsf_processed_evalu['pat_id'].values.tolist())
    ...
    df_processed_src = fm_io.read_file_to_data_frame \
        (src_dataset_folderpath + '/' + lab_src + '/%s-normality-matrix-processed.tab' %lab_src)
    df_processed_src.pop('pat_id')
    df_processed_src.pop(ylabel) # TODO?!
    processed_columns_src = df_processed_src.columns.values.tolist()

    df_src_stats = pd.read_csv(src_dataset_folderpath.replace('machine_learning', 'lab_statistics')
                               + '/' + 'summary-stats-bestalg-fixTrainPPV.csv', keep_default_na=False)
    best_alg = df_src_stats[(df_src_stats['lab']==lab_src) & (df_src_stats['fixTrainPPV']==0.95)]['best_alg'].values[0]

    classifier_src = SL.load_ML_model(lab=lab_src, alg=best_alg, dataset_folderpath=src_dataset_folderpath)
...
def transfer_labs(src_dataset='Stanford', dst_dataset='UCSF', lab_type='panel', cur_version='10000-episodes'):
    ...
    (lab_type ,src_dataset ,dst_dataset ,cur_version)
    ...
    for lab in labs:
        ...
        apply_src_to_dst(lab=lab, lab_type=lab_type,
                         ...
                         src_dataset_folderpath=ml_folder + 'data-%s-%s-%s' %(src_dataset, lab_type ,cur_version)
                         ,  # 'data-Stanford-%s-10000-episodes'%lab_type,
