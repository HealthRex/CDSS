...
def run_one_lab_local(lab, lab_type, data_source, version):
    ...
    file_organizer = LNL_sys.FileOrganizerLocal(lab_type=lab_type,
                                                 data_source=data_source,
                                                version=version)

    Xy_raw = file_organizer.get_raw_matrix(lab)

    X_train_raw, y_train = Utils.Split_Xy(Xy_raw)

    pipeline = Pipeline(memory=LNL_sys.get_cached_pipeline_filepath(lab=lab,
                                                                    lab_type=lab_type,
                                                                    data_source=data_source,
                                                                    version=version),
                        steps=
