...
(
	select 
		...
		['8']
			as specialtyIds, 	-- '10' -- dep_map.specialty = 'Gastroenterology' 
									-- '7' -- dep_map.specialty like 'Endocrin%'
									-- '14' -- dep_map.specialty like 'Hematology'
									-- '8' -- dep_map.specialty like 'ENT%'
		
		[9991028, 8002445, 8002280, 8601002]	-- Primary ENT departments (as opposed to sattelite clinics)
			as specialtyDepartmentIds,	
...
(
	...
	from `shc_core_2022.encounter` as enc 
		join `shc_core_2022.dep_map` as dep on enc.department_id = dep.department_id,
		params
	where dep.specialty_dep_c in UNNEST(params.specialtyIds)
	and dep.department_id in UNNEST(params.specialtyDepartmentIds)
