...
(
	select 
		...
		2019 as cohortYear,	-- Review for just one year (and beware of 202 for weird COVID patterns)
...
(
	...
	from procCountsPerSpecialtyDiagnosis, params
	where nPatientsProcPerSpecialtyDiagnosis > params.minPatientsForNonRareItems -- Ignore small cases by only looking at items that are ordered for more than 10 different patients
	order by
...
spacer AS (select null as tempSpacer)
