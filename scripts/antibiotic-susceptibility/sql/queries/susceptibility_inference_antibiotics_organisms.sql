...
--add piror culture sensitvity to implied table
...
Base_cohort AS (
  ...
  inner join `som-nero-phi-jonc101.fateme_db.antibiotic_cohort_temp` t using (anon_id,order_proc_id_coded)
...
AND
 lower(antibiotic) like any ('amikacin','aztreonam','ceftazidime','ciprofloxacin','colistin','gentamicin','metronidazole','tobramycin')
AND (susceptibility IS NULL OR susceptibility = '')
AND  order_proc_id_coded IN (
SELECT order_proc_id_coded
from `som-nero-phi-jonc101.Fatemeh_db.implied_suscept`
where UPPER(organism) LIKE 'STAPHYLOCOCCUS AUREUS'
AND lower(antibiotic) like 'penicillin'
AND (susceptibility='Susceptible' or implied_susceptibility='Susceptible')
); 
...
'cefepime','ceftriaxone','cefazolin','cefotaxime','cefotetan','cefoxitin','cefpodoxime','ceftaroline','cefuroxime','daptomycin','doripenem','ertapenem','imipenem','linezolid','meropenem','oxacillin','penicillin','tigecycline','vancomycin')
AND (susceptibility IS NULL OR susceptibility = '')
AND  order_proc_id_coded IN (
SELECT order_proc_id_coded
from `som-nero-phi-jonc101.Fatemeh_db.implied_suscept`
...
---------------------------------
--levofloxacin if missing then E moxifloxacin; if missing then .
------------------------------
UPDATE `som-nero-phi-jonc101.Fatemeh_db.implied_suscept`
SET implied_susceptibility = CASE 
    WHEN order_proc_id_coded IN (
...
-------------------------
--*moxifloxacin if missing then E levofloxacin
-------------------------
UPDATE `som-nero-phi-jonc101.Fatemeh_db.implied_suscept`
SET implied_susceptibility = CASE 
    WHEN order_proc_id_coded IN (
