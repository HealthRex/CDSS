...
class FeatureEngineer:
    ...
    def _get_demographics(self) -> Dict[str, Any]:
        ...
        # Age bins (one-hot encoded, only bins used by models as top-10 features)
        AGE_BINS = [
            ('demographics_age__18-24 years', 18, 24),
            ('demographics_age__25-34 years', 25, 34),
            ('demographics_age__35-44 years', 35, 44),
            ('demographics_age__55-64 years', 55, 64),
            ('demographics_age__75-84 years', 75, 84),
        ]
        for bin_name, low, high in AGE_BINS:
            features[bin_name] = 1 if (age is not None and low <= age <= high) else 0
    ...
    # Class-level cache for ADI lookup data (loaded once)
    _adi_lookup = None

    def _get_adi_score(self) -> Dict[str, float]:
        """
        Get Area Deprivation Index scores from pre-processed lookup file.

        Looks up patient ZIP code in adi_lookup.pkl (generated by preprocess_adi.py).
        Tries 9-digit ZIP first, falls back to 5-digit.
        Returns 0.0 if no ZIP or lookup file available.
        """
        features = {'adi_score': 0.0}

        postal_code = self.patient_data.get('postalCode', '')
        if not postal_code:
            return features

        # Load ADI lookup once (class-level cache)
        if FeatureEngineer._adi_lookup is None:
            adi_path = os.environ.get('ADI_LOOKUP_PATH', '')
            if not adi_path:
                model_dir = os.environ.get('MODEL_DIR', '')
                if model_dir:
                    adi_path = os.path.join(model_dir, 'adi_lookup.pkl')
            if adi_path and os.path.exists(adi_path):
                with open(adi_path, 'rb') as f:
                    FeatureEngineer._adi_lookup = pickle.load(f)
                logger.info(f"Loaded ADI lookup from {adi_path}")
            else:
                logger.warning(f"ADI lookup file not found at {adi_path}")
                return features

        adi = FeatureEngineer._adi_lookup

        # Normalize ZIP: remove dash, try 9-digit then 5-digit
        zip_clean = postal_code.replace('-', '')
        result = adi.get('zip9', {}).get(zip_clean)
        if result is None:
            result = adi.get('zip5', {}).get(zip_clean[:5])

        if result:
            natrank, staternk = result
            features['adi_score'] = float(staternk)
            features['adi_scores_adi_state_rank'] = float(staternk)
            # adi_scores_adi_score left as staternk for now — pending trainer confirmation
            # on whether this should be natrank, staternk, or natrank/10
        else:
            logger.warning(f"ZIP {postal_code} not found in ADI lookup")
