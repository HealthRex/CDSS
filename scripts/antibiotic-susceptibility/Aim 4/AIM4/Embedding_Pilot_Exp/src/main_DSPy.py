...
async def main_single_run(BASE_PATH, with_reference=False, size=20, sleep_per_task = 1.5):
    ...
    sample_data = load_data(size=size, random_state=42, pre_defined=True, with_reference=with_reference)
...
async def main():
    await main_single_run(BASE_PATH="src/DSPy_results_batch_1000_dedup_with_prev_msg", with_reference=False, size=1000, sleep_per_task=1.2)
    await main_single_run(BASE_PATH="src/DSPy_results_batch_1000_dedup_with_prev_msg_w_ref", with_reference=True, size=1000, sleep_per_task=1.6)
