...
select c.anon_id,
  ...
  TIMESTAMP_DIFF(c.order_time_jittered_utc, ap.procedure_time, day) as procedure_time_to_culturetime
  from antibiotic_cohort c left join all_procedure ap using (anon_id)
  where TIMESTAMP_DIFF(c.order_time_jittered_utc, ap.procedure_time, day) > 0
