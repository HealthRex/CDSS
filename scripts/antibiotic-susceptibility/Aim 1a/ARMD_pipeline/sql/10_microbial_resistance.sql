...
CREATE OR REPLACE TABLE `som-nero-phi-jonc101.antimicrobial_stewardship.microbiology_microbial_resistance_augmented` AS(
###############
...
microbial_resistant as (
  SELECT
    ...
    cs.organism,
    cs.antibiotic,
    cs.order_time_jittered_utc as recorded_resistant_time,
    TIMESTAMP_DIFF(c.order_time_jittered_utc,cs.order_time_jittered_utc,day) as resistant_time_to_culturetime
  FROM  base_cohort c 
  inner join `som-nero-phi-jonc101.shc_core_2023.culture_sensitivity` cs using (anon_id)
...
mr.organism,
mr.antibiotic,
mr.recorded_resistant_time,
mr.resistant_time_to_culturetime,
from base_cohort c  left join microbial_resistant mr using (anon_id,pat_enc_csn_id_coded,order_proc_id_coded,order_time_jittered_utc)
)
