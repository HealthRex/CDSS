-- Gets all antibiotic orders that affect label that happen within First two weeks hours
WITH er_admits AS (
...
WHERE affects_not_infected_label = 1
)
...
WHERE TIMESTAMP_DIFF(om.order_time, ea.er_admit_time, HOUR) BETWEEN 0 AND 24*14
