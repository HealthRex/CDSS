...
cohort_and_index_times AS (
  SELECT 
    abx.anon_id,
    abx.pat_enc_csn_id_coded,
    min(order_time) index_time
  FROM 
    `mining-clinical-decisions.abx.abx_orders_within_24_hrs` abx
  INNER JOIN
    `mining-clinical-decisions.abx.culture_orders_within_24_hrs` cult 
  USING
    (pat_enc_csn_id_coded)
  GROUP BY
...
has_no_prior_abx_2_weeks AS (
  SELECT DISTINCT 
    cit.pat_enc_csn_id_coded
  FROM
    cohort_and_index_times cit
  LEFT JOIN
    ...
    -- gives a little bit of wiggle room because I'm not sure how accurate order_start_time is. 
      (SELECT DISTINCT
       ...
       INNER JOIN (
         ...
         SELECT 
          om.anon_id, om.pat_enc_csn_id_coded,
          om.med_description, om.order_med_id_coded,
          om.discon_time_jittered_utc,
          CASE WHEN om.order_inst_utc IS NULL THEN omm.order_time_jittered_utc
          ELSE om.order_inst_utc END order_time
         FROM 
          `shc_core.order_med` om
         LEFT JOIN 
          `starr_datalake2018.order_med` omm
         USING
          (order_med_id_coded)
        ) om
       USING
        (anon_id)
       INNER JOIN
         `mining-clinical-decisions.abx.abx_types` abx_types
       USING
        (med_description)
       WHERE 
        TIMESTAMP_DIFF(ea.er_admit_time, om.order_time, HOUR) BETWEEN 1 AND 24*14 
       AND
        abx_types.affects_not_infected_label = 1) has_prior_order
  USING
    (pat_enc_csn_id_coded)
  WHERE
    has_prior_order.pat_enc_csn_id_coded IS NULL
),
    ...
    -- Finds all csns where cultures were ordered between 1 hours and 14 weeks before admit
     (SELECT DISTINCT
      ...
      USING
        (pat_enc_csn_id_coded)
      INNER JOIN
        `shc_core.order_proc` op
      USING
        (anon_id)
      INNER JOIN
        `mining-clinical-decisions.abx.culture_types` culture_types
      USING
        (description)
      WHERE
        TIMESTAMP_DIFF(ea.er_admit_time, op.order_time_jittered_utc, HOUR)
      BETWEEN
        1 AND 24*14
      AND
        culture_types.affects_not_infected_label = 1) has_prior_order
  USING
    (pat_enc_csn_id_coded)
  WHERE
    has_prior_order.pat_enc_csn_id_coded IS NULL
...
filtered_examples AS (
  ...
  FROM
    cohort_and_index_times cit
  INNER JOIN
    has_no_prior_abx_2_weeks 
  USING
    (pat_enc_csn_id_coded)
  INNER JOIN
    has_no_prior_cultures 
  USING
    (pat_enc_csn_id_coded)
),
...
last_abx_stop_time_if_started as (
  SELECT
    anon_id, pat_enc_csn_id_coded, max(abx_stop_time) abx_stop_time
  FROM 
    `mining-clinical-decisions.abx.abx_orders_given_and_stopped_2_weeks`
  WHERE
    was_given = 1
  GROUP BY
    anon_id, pat_enc_csn_id_coded
...
started_abx_stopped_within_day_after_cult_results_or_not_started AS (
  SELECT DISTINCT
    fe.pat_enc_csn_id_coded
  FROM
    filtered_examples fe
  LEFT JOIN
    last_abx_stop_time_if_started las
  USING
    (pat_enc_csn_id_coded)
  LEFT JOIN
    er_admits ea
  USING
    (pat_enc_csn_id_coded)
  WHERE
    las.abx_stop_time IS NULL -- its null for csn's where abx were never started
  OR 
    -- abx that were started must be stopped 2 days after er admit time
    las.abx_stop_time <= TIMESTAMP_ADD(ea.er_admit_time, INTERVAL 48 HOUR) 
),

-- Gets CSNS where we say patients were discharged with oral abx defined as:
-- There exists an order for oral abx within the interval
...
discharged_with_orals AS (
  ...
  INNER JOIN 
   (SELECT
      fe.pat_enc_csn_id_coded, max(adt.effective_time_jittered_utc) discharge_time
    FROM
      filtered_examples fe
    INNER JOIN
      `shc_core.adt` adt
    USING
      (pat_enc_csn_id_coded)
    GROUP BY
      fe.pat_enc_csn_id_coded) discharge
  USING
    (pat_enc_csn_id_coded)
  LEFT JOIN (
      -- Gets columns we need from order med and finds order time from 2018 extract when missing in 2020 extract 
  ...
  WHERE
    abx_types.is_oral = 1
  AND 
    om.order_time BETWEEN TIMESTAMP_SUB(discharge.discharge_time, INTERVAL 12 HOUR)
  AND
    TIMESTAMP_ADD(discharge.discharge_time, INTERVAL 24*5 HOUR)
),
...
has_pos_cult_ordered_2_weeks AS (
  ...
  USING
    (order_proc_id_coded)
  WHERE
    culture_types.affects_not_infected_label = 1 
  AND
    TIMESTAMP_DIFF(op.order_time_jittered_utc, ea.er_admit_time, HOUR) BETWEEN 0 AND 14*24
  AND
    cs.organism <> "COAG NEGATIVE STAPHYLOCOCCUS" 
  AND
    cs.organism NOT LIKE "%CANDIDA%" 
  AND
    cs.organism NOT IN ('HAEMOPHILUS INFLUENZAE', 'HAEMOPHILUS PARAINFLUENZAE')
),
...
USING
  (order_proc_id_coded)
WHERE
  culture_types.include = 1 
AND
  TIMESTAMP_DIFF(op.order_time_jittered_utc, ea.er_admit_time, HOUR) BETWEEN 0 AND 24
AND
  cs.organism <> "COAG NEGATIVE STAPHYLOCOCCUS" 
AND
  cs.organism NOT LIKE "%CANDIDA%" 
...
patient_died AS (
  ...
  USING
    (anon_id)
  WHERE
    demo.DEATH_DATE_JITTERED IS NOT NULL 
  AND
    DATE_DIFF(demo.DEATH_DATE_JITTERED , CAST(ea.er_admit_time AS DATE), DAY)
  BETWEEN 0 AND 14
),
...
infection_dx_code AS (
  ...
  ON 
    fe.pat_enc_csn_id_coded = dx.pat_enc_csn_id_jittered
  WHERE 
    icd9 LIKE "995.91%" -- Sepsis
...
-- Flags whether the patient had an operative note recorded within a window of index time
-- window defined as [-7 days, +7 days]
has_operative_note as (
  SELECT
      fe.pat_enc_csn_id_coded,
      MAX (
          CASE WHEN TIMESTAMP_DIFF(fe.index_time, cdoc.note_date_jittered_utc, DAY)
          BETWEEN -7 AND 7 AND note_type LIKE 'Operative%' THEN 1
          ELSE 0 END
      ) has_op_note,
  FROM
      filtered_examples fe
  LEFT JOIN 
      `mining-clinical-decisions.starr_datalake2018.clinical_doc_meta` cdoc
  ON
      anon_id = jc_uid
  GROUP BY
      pat_enc_csn_id_coded
...
SELECT 
  ...
  CASE WHEN cdoc.has_op_note = 0 THEN 1 ELSE 0 END no_op_notes
FROM
  filtered_examples fe
LEFT JOIN
  started_abx_stopped_within_day_after_cult_results_or_not_started abx
USING
  (pat_enc_csn_id_coded)
LEFT JOIN
  discharged_with_orals orals
USING
  (pat_enc_csn_id_coded)
LEFT JOIN
  has_pos_cult_ordered_2_weeks other_cult 
USING
  (pat_enc_csn_id_coded)
LEFT JOIN
  has_pos_cult current_cult
USING
  (pat_enc_csn_id_coded)
LEFT JOIN
  patient_died dead
USING
  (pat_enc_csn_id_coded) 
LEFT JOIN
  infection_dx_code dx
USING
  (pat_enc_csn_id_coded)
LEFT JOIN
    has_operative_note cdoc
USING 
    (pat_enc_csn_id_coded)