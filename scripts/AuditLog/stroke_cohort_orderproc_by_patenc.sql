-- med_proc -- laboratory -- update to innerjoin 

select  
   count(description) count, description 
from 
  `datalake_47618.order_proc`  as opt
inner join 
    ...
    group by 
        ...
        om.med_description,
        inpatient.pat_class
    order by emergencyAdmitTime) as stroke_cohort

on opt.pat_enc_csn_id_coded = stroke_cohort.pat_enc_csn_id_coded 
