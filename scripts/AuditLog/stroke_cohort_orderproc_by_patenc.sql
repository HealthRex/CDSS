-- med_proc -- laboratory -- updated to pat_enc_csn_id_coded instead of jc_uid
SELECT count(description) count, description from (
Select 
  jc_uid, pat_enc_csn_id_coded, ordering_date_jittered, description, display_name, cpt_code 
from 
...
where pat_enc_csn_id_coded in( 
  select pat_enc_csn_id_coded from(
   ...
   group by 
     jc_uid, pat_enc_csn_id_coded, ordering_date_jittered, display_name, cpt_code, description 
)
