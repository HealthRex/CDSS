...
Select jc_uid, dx_id, dx_name, icd9, icd10, start_date_jittered, pat_enc_csn_id_coded
from `datalake_47618.diagnosis_code` 
where jc_uid in( 
  select jc_uid as rit_uid from(
    select 
      op.jc_uid, op.pat_enc_csn_id_coded, 
      admit.event_type, admit.pat_class, 
      ...
      inpatient.pat_class as inptClass, 
      min(inpatient.effective_time_jittered) as inpatientAdmitTime
    from 
    ...
    where op.pat_enc_csn_id_coded = admit.pat_enc_csn_id_coded
