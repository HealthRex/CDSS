...
class FeatureMatrixFactory:
    ...
    def doLabCache(self):
        self._doLabCache = True

    def clearLabCache(self):
        self._labCache = None
        self._doLabCache = False
    ...
    def _processClinicalItemEvents(self, patientEpisodes, itemTimesByPatientId, clinicalItemNames, dayBins, label=None, features=None):
        ...
        if features != "pre":
            ...
            if len(dayBins) > 0:
                tempFile.write("\t")
                tempFile.write("\t".join("%s.%dd" % (postLabel, dayBin) for dayBin in postDayBins))
        tempFile.write("\n")
        ...
        for patientEpisode in patientEpisodes:
            ...
            if features != "pre":
                ...
                if len(dayBins) > 0:
                    tempFile.write("\t")
                    tempFile.write("\t".join([str(episodeData["%s.%dd" % (postLabel, dayBin)]) for dayBin in postDayBins]))
            tempFile.write("\n")
    ...
    def addLabResultFeatures(self, labNames, labIsPanel = True, preTimeDelta = None, postTimeDelta = None):
        ...
        print("1")
        labResults = None
        if (self._labCache != None):
            labResults = self._labCache
        else:
            labResults = self._queryLabResultsByName(labNames, labIsPanel)
        if (self._doLabCache == True):
            self._labCache = labResults
        print("2")
