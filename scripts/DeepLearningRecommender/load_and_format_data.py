...
def extract_features_responses(splits_padded, outdir):
    ...
    for name, split in zip(['train', 'dev', 'test'], [train, dev, test]):
        ...
        for patient in split:
            patient.drop(["patient_item_id", "patient_id", "clinical_item_id", "item_date", "encounter_id"], axis=1,
                         inplace=True)
...
def make_train_dev_test(data_dir, fname='rnn_data.txt', outdir='/home/ec2-user/cs230/scripts/DeepLearning/RNN/data/',
                        train_percent=0.9, dev_percent=0.05):
    '''
    ...
    train_patient_ids, dev_patient_ids, test_patient_ids = select_patients(all_patient_ids, (
    train_num_patients, dev_num_patients, test_num_patients))
    train, dev, test = variable_length_splits(all_data, all_patient_ids,
                                              (train_patient_ids, dev_patient_ids, test_patient_ids), outdir)

    return


def make_train_dev_test_v2(data_dir, in_prefix='rnn_data_',
                           outdir='/home/ec2-user/cs230/scripts/DeepLearning/RNN/data/', train_percent=0.9,
                           dev_percent=0.05, nfiles=1):
    '''
    Generates train,dev,and test set for RNN model.  
    A single training example comprises all clinical item orders for a given patient. 
    Therefore the shape of a single example will be (# of clinical item orders, # of features).
    ...
    for i in range(nfiles):
        ...
        train_patient_ids, dev_patient_ids, test_patient_ids = select_patients(all_patient_ids, (
        train_num_patients, dev_num_patients, test_num_patients))
        train, dev, test, out_counter = variable_length_splits(all_data, all_patient_ids,
                                                               (train_patient_ids, dev_patient_ids, test_patient_ids),
                                                               outdir, counter=out_counter)
