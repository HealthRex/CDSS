...
CREATE TABLE IF NOT EXISTS sim_patient_order
(
    ...
    CONSTRAINT sim_patient_order_pkey PRIMARY KEY (sim_patient_order_id),
    CONSTRAINT sim_patient_order_sim_user_fkey FOREIGN KEY (sim_user_id) REFERENCES sim_user(sim_user_id),
    CONSTRAINT sim_patient_order_sim_patient_fkey FOREIGN KEY (sim_patient_id) REFERENCES sim_patient(sim_patient_id),
    CONSTRAINT sim_patient_order_clinical_item_fkey FOREIGN KEY (clinical_item_id) REFERENCES clinical_item(clinical_item_id)
);
...
CREATE INDEX IF NOT EXISTS index_sim_patient_order_sim_user_patient_date ON sim_patient_order(sim_user_id, sim_patient_id, relative_time_start);	-- Natural sorting option to order by user, patient, then in chronological order of clinical items
CREATE INDEX IF NOT EXISTS index_sim_patient_order_sim_clinical_item ON sim_patient_order(sim_patient_id, clinical_item_id);	-- Natural sorting option to order by patient, then in chronological order of clinical items
...
ALTER TABLE sim_patient_order ADD COLUMN IF NOT EXISTS sim_state_id BIGINT NOT NULL DEFAULT 0;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'sim_patient_order' AND constraint_name = 'sim_patient_order_state_fkey'
  )
  THEN
    ALTER TABLE sim_patient_order ADD CONSTRAINT sim_patient_order_state_fkey FOREIGN KEY (sim_state_id) REFERENCES sim_state(sim_state_id);
  END IF;
