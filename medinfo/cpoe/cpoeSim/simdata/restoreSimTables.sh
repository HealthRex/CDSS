...
# Add data for specific test case info
#psql -f meningitis.dump.sql -h $DB_HOST -p $DB_PORT -U $DB_USER $DB_DSN
#psql -f afib-rvr.dump.sql -h $DB_HOST -p $DB_PORT -U $DB_USER $DB_DSN

# Individul test user interaction behavior (but some default ones are necessary for infrastructure)
