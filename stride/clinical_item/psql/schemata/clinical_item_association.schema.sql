...
-- Description: Core stat tracking table, record various association
--              statistics mostly counting up the number of times one item /
--              order follows another	within any patient's course.
...
-- Basic type of stats, counting up cooccurences.  Column names encoded by
-- duration in seconds the counting timeframe represents. Use DOUBLE PRECISION
-- / floating point values to allow for weighted pseudo-counts.
-- Number of times the subsequent item follows the primary clinical item within
-- _____ of event TIMESTAMP.
ALTER TABLE clinical_item_association ADD COLUMN count_0 DOUBLE PRECISION DEFAULT 0; -- 0 seconds
...
-- Number of times the subsequent item follows within any period of recorded time
ALTER TABLE clinical_item_association ADD COLUMN count_any DOUBLE PRECISION DEFAULT 0;
-- Sum of differences in event times of subsequent vs. primary items, expressed
-- in seconds.  This divided by count_any reflects the average time difference
ALTER TABLE clinical_item_association ADD COLUMN time_diff_sum DOUBLE PRECISION DEFAULT 0;
-- Sum of squared difference values as above.  Can be used with above to assess
-- variance of time differences (var = (SumSquares - Sum^2) / count_any)
ALTER TABLE clinical_item_association ADD COLUMN time_diff_sum_squares DOUBLE PRECISION DEFAULT 0;

-- Similar stats, but don't count duplicates within the timeframes, only
-- incrementing counts for the first subsequent item encountered. Expand time
-- windows for counting. Should always meet or exceed dataset timeframe so easy
-- to upscale without full recalc. More stats, but now count simply by the
-- existence of a patient with the combination and only count once per patient.
...
-- More stats, but now count by the existence of a patient encounter with the
-- combination and only count once per encounter.
